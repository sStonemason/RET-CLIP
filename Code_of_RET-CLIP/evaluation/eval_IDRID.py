...
class IDRiDDataset(data.Dataset):
    ...
    def get_label(self, filepath):
        ...
        if self.split == 'train':
            labelfile = self.label_dir + '/IDRiD_Disease Grading_Train_Labels.csv'
        elif self.split == 'valid':
            labelfile = self.label_dir + '/IDRiD_Disease Grading_Valid_Labels.csv'
        elif self.split == 'test':
            labelfile = self.label_dir + '/IDRiD_Disease Grading_Testing_Labels.csv'

        df = pd.read_csv(labelfile)
        id = str(filepath.split('/')[-1].split('.')[0])
        ret = torch.tensor(df.loc[df['Image name'] == id].values[0][1])
        return ret
...
def eval_multiLabelCls_ViT(model, device):
    ...
    if TRAIN:
        ...
        train_dataset = IDRiDDataset(data_dir=DATA_DIR, label_dir=LABEL_DIR, split='train', imsize=224)
        val_dataset = IDRiDDataset(data_dir=DATA_DIR, label_dir=LABEL_DIR, split='valid', imsize=224)
        test_dataset = IDRiDDataset(data_dir=DATA_DIR, label_dir=LABEL_DIR, split='test', imsize=224)
        ...
        for epoch in range(50):
            ...
            for step in tqdm(range(len(data_iter))):
                ...
                # adjust_learning_rate(optimizer, epoch, 50, 10, 2e-6, 1e-6)
