...
class APTOSDataset(data.Dataset):
    ...
    def get_label(self, filepath):
        ...
        if self.split == 'train':
            labelfile = self.label_dir + '/train.csv'
        elif self.split == 'valid':
            labelfile = self.label_dir + '/valid.csv'
        elif self.split == 'test':
            labelfile = self.label_dir + '/test.csv'

        df = pd.read_csv(labelfile)
        id = str(filepath.split('/')[-1].split('.')[0])
        ret = torch.tensor(df.loc[df['id_code'] == id].values[0][1])
        return ret
...
def eval_multiLabelCls_ViT(model, device):
    ...
    if TRAIN:
        ...
        train_dataset = APTOSDataset(data_dir=DATA_DIR, label_dir=LABEL_DIR, split='train', imsize=224)
        val_dataset = APTOSDataset(data_dir=DATA_DIR, label_dir=LABEL_DIR, split='valid', imsize=224)
        ...
        test_dataset = APTOSDataset(data_dir=DATA_DIR, label_dir=LABEL_DIR, split='test', imsize=224)
        ...
        for epoch in range(50):
            ...
            for step in tqdm(range(len(data_iter))):
                ...
                # adjust_learning_rate(optimizer, epoch, 50, 20, 1.5e-6, 1e-6)
            ...
            with torch.no_grad():
                ...
                for step in tqdm(range(len(data_iter))):
                    ...
                    imgs = image.to(device)
                    # label = F.one_hot(label.to(torch.int64), num_classes=5)
                    image_features = model(imgs, None, None)
                    # pred_test = model(imgs)
                    # pred_test = nn.Softmax(dim=1)(pred_test)
...
def main():
    ...
    for i in range(5):
        ...
        log_path = ''
        log_path = os.path.join(log_path, "".format(i + 1, time_suffix))
