...
class PAPILADataset(data.Dataset):
...
def eval_multiLabelCls_ViT(model, device):
    ...
    if TRAIN:
        ...
        train_dataset = PAPILADataset(data_dir=DATA_DIR, label_dir=LABEL_DIR, split='train', imsize=224)
        val_dataset = PAPILADataset(data_dir=DATA_DIR, label_dir=LABEL_DIR, split='valid', imsize=224)
        test_dataset = PAPILADataset(data_dir=DATA_DIR, label_dir=LABEL_DIR, split='test', imsize=224)
        ...
        for epoch in range(50):
            ...
            for step in tqdm(range(len(data_iter))):
                ...
                # adjust_learning_rate(optimizer, epoch, 50, 10, 2e-6, 1.3e-6)
