...
def get_loss(model, img_l, img_r, texts, loss_img, loss_txt, eos_indices, args, accum_image_features=None,
    ...
    if args.report_training_batch_acc:
        i2t_acc = (logits_per_left_image.argmax(-1) == ground_truth_left).sum() / len(logits_per_left_image)
        t2i_acc = (logits_per_left_text.argmax(-1) == ground_truth_left).sum() / len(logits_per_left_text)
        acc = {"i2t": i2t_acc, "t2i": t2i_acc}

    # if args.distllation:
    #     total_loss += kd_loss * args.kd_loss_weight

    return total_loss, acc, left_loss, right_loss, global_loss
            ...
            if args.precision == "amp":
                with autocast():
                    if args.distllation:
                        ...
                        total_loss, acc, left_loss, right_loss, global_loss = get_loss(model, img_l, img_r, texts, loss_img, loss_txt, eos_indices, args)
...
def evaluate(model, data, epoch, args, steps, vision_model):
    ...
    if vision_model in ['RN50']:
        epoch_main, best_auc, best_map, best_epoch = eval_multiLabelCls_RN50(copy_model, epoch + 1,
        ...
        epoch_main, best_auc, best_map, best_epoch = eval_multiLabelCls_ViT(copy_model, epoch + 1,
                                                                            cuda_rank=args.local_device_rank)
        logging.info(
            f"MultiLabelCls Validation Result (epoch {epoch_main}) | "
            f"Best epoch_valid: {best_epoch:.6f} | "
