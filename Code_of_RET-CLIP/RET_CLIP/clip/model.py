...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        self.use_rn_for_embed = use_rn_for_embed
        if self.use_rn_for_embed:
            self.resnet34 = RestNet34()
            print('use resnet for embedding...')
    ...
    def forward(self, x: torch.Tensor, mask_ratio: float = 0.0):
        if self.resnet34 is not None:
            x_rn34 = self.resnet34(x)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisualTransformer(
                ...
                use_rn_for_embed=use_rn_for_embed
            )
    ...
    def initialize_parameters(self):
        ...
        if isinstance(self.visual, VisualTransformer):
            if self.visual.use_rn_for_embed:
                for param in self.visual.resnet34.parameters():
                    nn.init.zeros_(param)
